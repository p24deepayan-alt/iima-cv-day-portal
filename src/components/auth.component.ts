...
type AuthMode = 'login' | 'register-student' | 'register-reviewer';
      ...
      <div class="flex-1 overflow-auto bg-iima-warm relative">
        <div class="min-h-full flex items-center justify-center p-8">
          <div class="w-full max-w-md bg-white p-8 md:p-10 shadow-xl border-t-4 border-iima-red">
            ...
            <div class="mb-8">
              <h2 class="text-2xl font-serif font-bold text-slate-800 mb-1">
                {{ mode() === 'login' ? 'Portal Login' : 'Account Registration' }}
              </h2>
            ...
            <div class="flex mb-8 border-b border-slate-200">
              <button 
                (click)="setMode('login')" 
                [class.text-iima-blue]="mode() === 'login'" 
                [class.border-iima-blue]="mode() === 'login'" 
              ...
              <button 
                (click)="setMode('register-student')" 
                [class.text-iima-blue]="mode() === 'register-student'" 
                [class.border-iima-blue]="mode() === 'register-student'"
                [class.font-bold]="mode() === 'register-student'"
                class="px-4 pb-3 text-sm text-slate-500 border-b-2 border-transparent transition-all hover:text-iima-blue">
                PGP1
              </button>
              <button 
                (click)="setMode('register-reviewer')" 
                [class.text-iima-blue]="mode() === 'register-reviewer'" 
                [class.border-iima-blue]="mode() === 'register-reviewer'" 
                [class.font-bold]="mode() === 'register-reviewer'"
                class="px-4 pb-3 text-sm text-slate-500 border-b-2 border-transparent transition-all hover:text-iima-blue">
                PGP2
              </button>
            </div>
            ...
            <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-5">
              ...
              @if (mode() !== 'login') {
                ...
                <div>
                  <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide mb-1.5">Email Address</label>
                  <input type="email" formControlName="email" class="w-full p-3 bg-slate-50 border border-slate-300 rounded-sm focus:border-iima-blue focus:ring-1 focus:ring-iima-blue outline-none">
                </div>
                 <div>
...
export class AuthComponent {
  ...
  submit() {
    ...
    if (m === 'login') {
      ...
      const role = m === 'register-student' ? 'student' : 'reviewer';
